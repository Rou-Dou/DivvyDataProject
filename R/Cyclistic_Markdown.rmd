...
```{r data-import, results=FALSE, warning=FALSE, class.source='fold-hide'}
...
and also ride more frequently during the week day, while casual riders
...
a sample from the main dataset d_ride_table and creating a plot for
which hours of the day casual/members use bikes the most. The margin of
error for a sample of 10000 for this population size at 95% confidence
is 0.96%

```{r}
d_ride_table_sample <- sample_n(d_ride_table,10000) %>%
  subset(select = c(member_casual, started_at,ended_at)) %>%
  mutate(hour_of_ride = format(as.POSIXct(started_at), format = "%H"))
```

```{r, echo=FALSE}
  ggplot(d_ride_table_sample, aes(x=hour_of_ride, fill = member_casual,
                                  group = member_casual)) +
    geom_bar(aes(y=after_stat(count)), position = position_dodge(0.9)) +
